...
// clear will spwan a copy of passline that waits in a detached background
// process group until the timeout is expired. It will then compare the contents
// of the clipboard and erase it if it still contains the data passline copied
// to it.
func clear(ctx context.Context, content []byte, timeout int) error {
	...
	cmd.Env = append(os.Environ(), "PASSLINE_UNCLIP_CHECKSUM="+hash)
	if !ctxutil.IsNotifications(ctx) {
		cmd.Env = append(cmd.Env, "PASSLINE_NO_NOTIFY=true")
	}
	return cmd.Start()
}
...
func walkFn(pid int, killFn func(int)) {
	...
	// any unrelated (passline) processes
	args := bytes.Split(cmdline, []byte{0})
	if len(args) < 2 {
		return
	}
	// the commandline should start with "passline"
