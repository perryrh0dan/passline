...
func TestLevenshteinDistanceSameLength(t *testing.T) {
	distance := LevenshteinDistance("maus", "haus")

	if distance != 1 {
		t.Errorf("LevenshteinDistance() = %d; wanted length %v", distance, 3)
	}
}

func TestLevenshteinDistanceDifferenceLength(t *testing.T) {
	distance := LevenshteinDistance("flake", "brakes")
...
func TestFindClosestSubstringMass(t *testing.T) {
	...
	start := time.Now()
	for _, v := range items {
		LevenshteinDistanceSubstring("flake", v)
	}
